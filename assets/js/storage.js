...
// + GitHub persistence via Netlify function
// ============================================
...
window.RapidWoo.Storage = {
  ...
  async getProducts() {
    try {
      ...
      try {
        ...
        console.warn('âš Ã¯Â¸Â Could not fetch products.json:', fetchError.message);
      }
  ...
  /**
   * Save products to USER storage (highest priority) - LOCAL ONLY
   */
  saveProducts(data) {
    try {
      const keys = window.RapidWoo.Config.STORAGE_KEYS;
      ...
      console.error('Ã¢ÂÅ’ Error saving products:', error);
      
      // If localStorage is full, try to clear old data
      if (error.name === 'QuotaExceededError') {
        console.warn('âš Ã¯Â¸Â Storage quota exceeded, clearing old data...');
        this.clearDemoData();
        try {
          ...
          console.error('Ã¢ÂÅ’ Still failed after clearing:', retryError);
          return false;
        }
      }
      return false;
  ...
  /**
   * =============================================
   * NEW: Save products to GitHub via Netlify function
   * =============================================
   * @param {Object} data - { products: [...] }
   * @param {String} commitMessage - Optional commit message
   * @returns {Object} - { success, commit, productCount } or { error }
   */
  async saveToGitHub(data, commitMessage = null) {
    try {
      const endpoint = window.RapidWoo.Config.API.GITHUB_SAVE;
      
      if (!endpoint) {
        throw new Error('GitHub save endpoint not configured');
      }

      const products = data.products || data;
      
      if (!Array.isArray(products)) {
        throw new Error('Invalid data: products array required');
      }

      console.log('ðŸŒ Saving to GitHub...', products.length, 'products');
      ...
      const response = await fetch(endpoint, {
        ...
        body: JSON.stringify({
          products: products,
          message: commitMessage || `Update products via RapidWoo Editor - ${new Date().toLocaleString()}`
        })
      });
      ...
      console.log('âœ… Saved to GitHub:', result);
      ...
      console.error('Ã¢ÂÅ’ GitHub save failed:', error);
      return {
        success: false,
        error: error.message
      };
  ...
  async loadDemoProducts() {
    try {
      ...
      console.error('Ã¢ÂÅ’ Error loading demo products:', error);
      return { products: window.RapidWoo.Config.DEMO_PRODUCTS };
    }
  },
  ...
  clearAll() {
    ...
    console.log('Ã°Å¸â€”â€˜Ã¯Â¸Â Cleared all product data');
  },
  ...
  clearDemoData() {
    ...
    console.log('Ã°Å¸â€”â€˜Ã¯Â¸Â Cleared demo data');
  },
  ...
  exportJSON(data, filename = 'products.json') {
    try {
      ...
      console.error('Ã¢ÂÅ’ Export failed:', error);
      return false;
    }
  },
  ...
  async importJSON(file) {
    try {
      ...
      console.error('Ã¢ÂÅ’ Import failed:', error);
      throw error;
    }
  },
...
console.log('âœ… RapidWoo Storage loaded (with GitHub support)');
