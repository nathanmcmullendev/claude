...
async function loadDemoFromFile() {
  try {
    ...
    console.error('[editor] Failed to load products.json:', err);
    return [];
  }
}

async function loadDummyFromFile() {
  try {
    const res = await fetch('/data/dummy-products.json', { cache: 'no-store' });
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const json = await res.json();
    return Array.isArray(json.products) ? json.products : [];
  } catch (err) {
    console.error('[editor] Failed to load dummy-products.json:', err);
    return [];
  }
}
...
function bindToolbar() {
  ...
  Utils.q('#btn-load-demo').addEventListener('click', async () => {
    ...
    safeSaveProducts();
    renderTable();
    Utils.showToast('Products refreshed from server', 'success');
  });
  ...
  Utils.q('#btn-load-dummy')?.addEventListener('click', async () => {
    ...
    const dummyProducts = await loadDummyFromFile();
    if (dummyProducts.length === 0) {
      Utils.showToast('Could not load demo products. Check that /data/dummy-products.json exists.', 'error');
      return;
    }
    App.products = normalizeAllProducts(dummyProducts);
    // Only save to localStorage (NOT to server) - user must click "Save to GitHub" to overwrite products.json
    Storage.saveProducts({ products: App.products });
    renderTable();
    Utils.showToast(`Loaded ${App.products.length} demo products. Click "Save to GitHub" to save permanently.`, 'success');
  });
